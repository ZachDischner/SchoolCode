...
for ii = 1:num_iterations
    ...
    for jj = 1:length(time)
           ...
           Xstar = StatePhi(:,1:18);
%            XstarCell   = num2cell(Xstar(jj,:));
%            [x(jj),y(jj),z(jj),xdot(jj),ydot(jj),zdot(jj)...
%             ,uE,J2,Cd,Xsite1(jj),Ysite1(jj),Zsite1(jj),...
%             Xsite2(jj),Ysite2(jj),Zsite2(jj),...
%             Xsite3(jj),Ysite3(jj),Zsite3(jj)] = XstarCell{:};
            x       = Xstar(:,1);
            y       = Xstar(:,2);
            z       = Xstar(:,3);
            xdot    = Xstar(:,4);
...
%             Cd(jj)      = X(9,jj);
    ...
    for jj = 1:length(rho_obs)
        ...
        if station(jj) == 101
            ...
            % Find H Tilde
            %---------------------------------------------
            Htilde = Htilde_2x12(Xsite(jj),Ysite(jj),Zsite(jj),theta(jj),theta_dot,x(jj),xdot(jj),y(jj),ydot(jj),z(jj),zdot(jj));
            %---------------------------------------------
