% function Xstar0 = KalmanFilter_DMC()
% Compute new xhat
...
num_iterations = 3;
 for ii = 1:num_iterations
    ...
    %---------------------------------------------
        ...
        Beta    = 0.002;
        sigma   = 0.067;
        Q_ECI = sigma.^2*eye(3,3)*1e-6;
        Q(:,:,jj)     = [Q_ECI zeros(3,15); zeros(15,18)];
        %---------------------------------------------
        ...
        %---------------------------------------------
        if jj > 1
            delta_t = time(jj) - time(jj-1);

            
            Phi_step= Phi(:,:,jj)/Phi(:,:,jj-1);
            m(jj) = exp(-Beta*(time(jj) - time(jj-1)));
            
            Gamma = sqrt(sigma^2/(2*Beta)*(1-m(jj)^2))*[delta_t/2*eye(3,3) zeros(3,15); eye(3,3) zeros(3,15); zeros(12,18)];
            Etabar(:,jj) = m(jj)*Etahat(:,jj-1);  % PhiStep???
            ...
            Pbar(:,:,jj)   = Phi_step*P(:,:,jj-1)*Phi_step' + Gamma*Q(:,:,jj-1)*Gamma';           
        else
            m(jj) = exp(-Beta*(time(jj)));
            Gamma = sqrt(sigma^2/(2*Beta)*(1-m(jj)^2))*[eye(3,3) zeros(3,15); eye(3,3) zeros(3,15); zeros(12,18)];
        end
        %---------------------------------------------
        ...
        % Kalman Gain
        %---------------------------------------------
        K1          = Pbar(:,:,jj)*Htilde'*inv(Htilde*Pbar(:,:,jj)*Htilde' + R);
        %---------------------------------------------
        
        
        if jj > 1
            Etahat(:,jj) = Etabar(:,jj) + K1*([rho_obs(jj);rhodot_obs(jj)]-Htilde*Etabar(:,jj));
        else
            Etahat(:,jj) = zeros(18,1);
        end
        
        % Measurement Update
        %---------------------------------------------
        xhat(:,:,jj) = xbar(:,:,jj) + K1*(y1(:,jj) - Htilde*xbar(:,:,jj));
         P(:,:,jj) = (eye(size(K1*Htilde)) - K1*Htilde)*Pbar(:,:,jj)*(eye(size(K1*Htilde))-K1*Htilde)' + K1*R*K1';
%        P(:,:,jj) = (eye(size(K1*Htilde)) - K1*Htilde)*P(:,:,jj);
%         P(:,:,jj) = ComputeP(Htilde,P(:,:,jj),R,Xstar(jj,1:18),ystar,'kalman');
        %---------------------------------------------
