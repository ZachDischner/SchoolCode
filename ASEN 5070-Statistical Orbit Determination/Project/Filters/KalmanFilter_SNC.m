% function Xstar0 = KalmanFilter_SNC()
% Compute new xhat
 ...
 for ii = 1:num_iterations
    ...
    %---------------------------------------------
        ...
        if jj > 1
            ...
            % Add Process Noise
            %---------------------------------------------
            sigma_acceleration = 0.005/sqrt(2);
            Q_ECI = sigma_acceleration.^2*eye(3,3);
            
            delta_t     = (time(jj) - time(jj-1));
            Q           = [Q_ECI zeros(3,15); zeros(15,18)];
            
            Gamma       = delta_t*[delta_t/2*eye(3,3) zeros(3,15); eye(3,3) zeros(3,15); zeros(12,18)];
%             Gamma       = delta_t .* [delta_t.*eye(3,3);eye(3,3);zeros(12,3)];
%             Q           = 0 .* (P(4:6,4:6,jj-1));
            if jj > 30 && ii >2
                Pbar(:,:,jj)   = Phi_step*P(:,:,jj-1)*Phi_step' + Gamma*Q*Gamma';
            else
                Pbar(:,:,jj)   = Phi_step*P(:,:,jj-1)*Phi_step';
            end
