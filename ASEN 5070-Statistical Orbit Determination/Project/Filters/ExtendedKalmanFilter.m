...
%              Phi_step= Phi(:,:,jj)/Phi(:,:,jj-1);
            ...
            if jj > 2
                if station(jj) ~= station(jj+1) || station(jj) ~= station(jj-1) || station(jj) ~= station(jj+2)|| station(jj) ~= station(jj+3)
                    StSw = [StSw;jj,1];
                    factor = .001;
                    P(:,:,jj)=P(:,:,jj).*factor;
                end
